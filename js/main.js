...
$(document).ready( function() {
 ...
 '<td>' + patient.room + '</td> <td>' + med.medName + '</br> <span style="font-style: italic;">' + med.dosage + '</span></td>' +
         '<td ><div id="errorMessage' + patientIndex + medIndex +'" style="color: red; display: none; padding-bottom: 10px; font-size: 10;">Incorrect dosage </div> <form role="form" class ="dosageForm" id="dosageForm' + patientIndex + medIndex +'"><div class="form-group"><input type="text" class="form-control dosage_delivered" id="dosageConfirm' + patientIndex + medIndex +'" placeholder="Dosage Delivered" style="font-size: 12;"></div><div class="form-group"><input type="checkbox" class="deliveredBox" id="box' + patientIndex + medIndex + '" style= "display: none;"><button type="submit" class="btn btn-default dosage_delivered" id="confirm' + patientIndex + medIndex +'">Confirm</button></div></form></td></tr>';
    ...
    function colorBoxes(){
      ...
      for(var i=1, len=tbl.rows.length; i<len; i++){ //start at row one to preserve header
        ...
        if (localStorage.getItem(checkbox.id.toString()) == 'true'){
          ...
          $("#dosageConfirm"+patientIndex+medIndex).attr('disabled', "disabled");
          $("#confirm"+patientIndex+medIndex).attr('disabled', "disabled");
          $("#confirm"+patientIndex+medIndex).css('background-color','#008073');
          $("#confirm"+patientIndex+medIndex).css('border-color','#008073');
